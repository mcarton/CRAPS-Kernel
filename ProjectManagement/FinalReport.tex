  ...
  \chapter{Validation phase}
    \section{Tests}
      ...
      \subsection{Processor}
        The processor already had some tests. We took care of their validity as
        we modified it.

        We also made more tests for each feature we have added.
      ...
      \subsection{Compiler}
        ...
        The other kind of tests test if the compiler generates valid code. These
        tests are not automated as we need to upload the code on the board after
        the compilation. Most of these tests are expected to produce a specific
        output on the 7-segment display when then succeed.

      \section{Kernel}
        We have made less tests for the kernel itself but it is the component we
        humans interact with and the kernel does not show unexpected behaviours.
        ...
        the kernel shows is the ultimate test.
