...
public class SHDLPredefinedRamsReadFirst extends SHDLPredefinedOccurence {

	private int nbitAddr;
	private int nbitData;

	public SHDLPredefinedRamsReadFirst(SHDLModuleOccurence moduleOccurence, Pattern namePattern) {
		super(moduleOccurence, namePattern);
	}

	public String toString() {
		return ("mo=" + getModuleOccurence() + ", nbitAddr=" + nbitAddr);
	}
	...
	public boolean check(boolean ok, SHDLModule module, PrintStream errorStream) {
		...
		nbitAddr = log2(nbadd);
		if (nbitAddr == -1) {
			errorStream.println("** " + module.getFile().getName() + ":" + moduleOccurence.getBeginLine() + ": predefined RAM module '" + moduleOccurence.getName() + "' : the size must be a power of 2. Examples are: 'rams_read_first16Kx32', 'rams_read_first256x8', 'rams_read_first4Mx16'");
			return false;
		}
	...
	public String getVHDLDefinition() {
		...
		sb.append(SHDLModule.newline);
		sb.append("-- single-port RAM in read-first mode" + SHDLModule.newline);
		sb.append(SHDLModule.newline);
